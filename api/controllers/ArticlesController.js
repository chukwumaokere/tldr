  ...
  updateArticle: async function(req, res) {
    ...
    if (!articleExists) {
      return res.badRequest(
        "No article found with that name. Please create the article first, or try again with an existing article name"
      )
    }

    if (updateMethod === undefined) {
      return res.badRequest(
        "updateMethod must be defined. Options are 'append' & 'overwrite'"
      )
    }
    ...
    if (updateMethod === "append") {
      if (newContent && newSources) {
        await sails.sendNativeQuery(
          `UPDATE articles SET content = $1, sources = $2, updatedAt = $3 WHERE articlename = $4`,
          [appendedContent, appendedSources, updatedAt, articleName]
        )
      } else if (newContent) {
        content = newContent
        await sails.sendNativeQuery(
          `UPDATE articles SET content = $1, updatedAt = $2 WHERE articlename = $3`,
          [appendedContent, updatedAt, articleName]
        )
      } else if (newSources) {
        sources = newSources
        await sails.sendNativeQuery(
          `UPDATE articles SET sources = $1, updatedAt = $2 WHERE articlename = $3`,
          [appendedSources, updatedAt, articleName]
        )
      } else {
        res.badRequest(
          "You must provide data for the update request to proceed"
        )
      }
    ...
    if (updateMethod === "overwrite") {
      if (newContent && newSources) {
        ...
        await sails.sendNativeQuery(
          `UPDATE articles SET content = $1, sources = $2, updatedAt = $3 WHERE articlename = $4`,
          [content, sources, updatedAt, articleName]
        )
      ...
      return res.badRequest({
        success: false,
        message:
          "Either no method was provided or an incorrect method was provided. Please provide a method of either 'append' or 'overwrite' in your JSON request"
      })
    }
  },
